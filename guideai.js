...
class GuideAI {
    ...
    updateCharCount() {
        ...
        if (count > maxLength * 0.9) {
            this.charCount.className = 'char-counter text-danger fw-bold';
        } else if (count > maxLength * 0.8) {
            this.charCount.className = 'char-counter text-warning';
        } else {
            this.charCount.className = 'char-counter text-muted';
        }
    ...
    formatBotResponse(content) {
        ...
        // Check if content is already properly formatted HTML from Parsedown
        ...
        // If content is already properly formatted by Parsedown, just enhance special terms
        if (hasParsedownHtml) {
            console.log('🔧 Content is already properly formatted by Parsedown - minimal enhancement only');
            
            // Only enhance special education terms and phone numbers without adding conflicting styles
            ...
            console.log('✅ Applied minimal enhancements to Parsedown HTML');
            return finalContent;
        }
        
        // For plain text content, apply enhanced markdown-style formatting
        let formattedContent = content;
        
        // Convert basic markdown headers with enhanced styling
        ...
        // Convert bold and italic text with enhanced styling
        formattedContent = formattedContent
        ...
        // Convert lists to clean HTML with enhanced styling and smart icons
        formattedContent = formattedContent
            ...
            // Handle any remaining asterisks that might not have been caught
        ...
        // Enhance important terms with special styling
        formattedContent = formattedContent
            .replace(/(IEP|IDEA|FAPE|504 Plan)/gi, '<span class="special-term">$1</span>')
            .replace(/(\d{3}-\d{3}-\d{4}|\(\d{3}\)\s*\d{3}-\d{4})/g, '<span class="phone-number">📞 $1</span>');
        ...
        console.log('✅ Applied enhanced formatting to plain text content');
    ...
    async handleEnhancedSubmit(message = null) {
        ...
        console.log('🔄 About to show enhanced typing indicator...');
        ...
        try {
            ...
            if (response && response.success) {
                ...
                // Check for different response structures with detailed logging
                if (response.data && response.data.content) {
                    console.log('✅ Found content in response.data.content');
                    content = response.data.content;
                } else if (response.data && typeof response.data === 'string') {
                    console.log('✅ Found content in response.data (string)');
                    content = response.data;
                } else if (response.result && response.result.mega_response) {
                    console.log('✅ Found content in response.result.mega_response');
                    content = response.result.mega_response;
                } else if (response.result && typeof response.result === 'string') {
                    console.log('✅ Found content in response.result (string)');
                    content = response.result;
                } else if (response.content) {
                    console.log('✅ Found content in response.content');
                    content = response.content;
                } else if (response.response) {
                    console.log('✅ Found content in response.response');
                    content = response.response;
                } else if (response.message) {
                    console.log('✅ Found content in response.message');
                    content = response.message;
                } else if (response.text) {
                    console.log('✅ Found content in response.text');
                    content = response.text;
                } else {
                    console.warn('⚠️ No content found in any expected field, using fallback');
                    console.log('🔍 Available response fields:', Object.keys(response));
                    // Try to extract any string content from the response
                    const stringFields = Object.values(response).filter(val => typeof val === 'string' && val.length > 10);
                    if (stringFields.length > 0) {
                        console.log('✅ Found string content in response:', stringFields[0].substring(0, 100) + '...');
                        content = stringFields[0];
                    } else {
                        content = 'I apologize, but I received an invalid response format. Please try asking your question again.';
                    }
                }
                ...
                console.log('📝 Content contains HTML tags:', /<[^>]*>/g.test(content));
                console.log('📝 Content preview (first 300 chars):', content.substring(0, 300));
                console.log('📝 Calling addEnhancedMessage with type: bot');
                
                // Clean and prepare content
                const cleanedContent = this.cleanAIResponseContent(content);
                console.log('📝 Cleaned content preview:', cleanedContent.substring(0, 300));
                
                this.addEnhancedMessage(cleanedContent, 'bot');
                console.log('📝 addEnhancedMessage completed');
                this.announceToScreenReader('GuideAI responded to your question');
            } else {
    ...
    addEnhancedMessage(content, type) {
        ...
        messageDiv.style.cssText = 'display: block; width: 100%; box-sizing: border-box; height: auto; overflow: visible;';
        ...
        if (type === 'user') {
            ...
            messageDiv.innerHTML = `
                <div class="bot-message" style="display: block; margin: 10px 0; padding: 15px; background-color: #ede7f6; border-radius: 12px; border-left: 4px solid #6f42c1; min-height: 60px; height: auto; width: 100%; box-sizing: border-box; overflow: visible;">
                    <div class="message-content" style="display: block; min-height: 40px; height: auto; overflow: visible;">
                        <div style="display: flex; align-items-start; margin-bottom: 8px;">
                            <div style="flex-grow: 1;">
                                <small style="color: #6c757d; display: block; margin-bottom: 5px;">
                                    GuideAI Assistant
        ...
        console.log('🔧 messageDiv properties after append:', {
            ...
            clientHeight: messageDiv.clientHeight,
            scrollHeight: messageDiv.scrollHeight,
    ...
    showEnhancedTypingIndicator() {
        ...
        typingDiv.style.cssText = 'display: block; width: 100%; box-sizing: border-box; height: auto; overflow: visible;';
        typingDiv.innerHTML = `
            <div class="bot-message" style="display: block; margin: 10px 0; padding: 15px; background-color: #f0f0f0; border-radius: 12px; border-left: 4px solid #6f42c1; height: auto; width: 100%; box-sizing: border-box; overflow: visible;">
                <div class="message-content" style="display: block; height: auto; overflow: visible;">
                    <div class="d-flex align-items-center">
                        <div class="ai-indicator me-2">
                            <span class="ai-name">GuideAI Assistant</span>
                        </div>
                    </div>
                    <div class="typing-animation mt-2" style="display: flex; align-items: center; gap: 8px;">
                        <span style="width: 8px; height: 8px; background-color: #6f42c1; border-radius: 50%; display: inline-block; animation: typing 1.4s infinite ease-in-out; animation-delay: 0s;"></span>
                        <span style="width: 8px; height: 8px; background-color: #6f42c1; border-radius: 50%; display: inline-block; animation: typing 1.4s infinite ease-in-out; animation-delay: 0.2s;"></span>
                        <span style="width: 8px; height: 8px; background-color: #6f42c1; border-radius: 50%; display: inline-block; animation: typing 1.4s infinite ease-in-out; animation-delay: 0.4s;"></span>
                    </div>
                    <div style="margin-top: 8px;">
                        <span class="typing-text" style="color: #6c757d; font-size: 0.9rem;">Processing your question...</span>
                    </div>
                </div>
            </div>
            <style>
        ...
        if (this.chatMessages) {
            ...
            console.error('❌ chatMessages element not found for typing indicator');
        }
    }

    hideEnhancedTypingIndicator() {
        console.log('🔄 Hiding enhanced typing indicator...');
        const typingIndicator = document.getElementById('enhancedTypingIndicator');
        if (typingIndicator) {
            typingIndicator.remove();
            console.log('✅ Enhanced typing indicator removed');
        } else {
            console.log('⚠️ Enhanced typing indicator not found to remove');
        }
        
        // Also remove any old typing indicators
        const oldIndicator = document.getElementById('typingIndicator');
        if (oldIndicator) {
            oldIndicator.remove();
            console.log('✅ Old typing indicator removed');
        }
    }
    ...
    resetVoiceButton() {
        ...
        if (this.userInput) {
            ...
            this.userInput.placeholder = this.getTranslation('input_placeholder') || 'Ask about IEPs, accommodations, your rights, or any special education topic...';
        }
    }

    // Clean AI response content to handle raw HTML issues
    cleanAIResponseContent(content) {
        console.log('🧹 Cleaning AI response content...');
        
        if (!content || typeof content !== 'string') {
            console.warn('⚠️ Invalid content received:', content);
            return 'Sorry, I received an invalid response. Please try again.';
        }
        
        // Check if content looks like raw HTML/CSS/JavaScript code
        const htmlPatterns = [
            /<\!DOCTYPE html>/i,
            /<html[^>]*>/i,
            /<head[^>]*>/i,
        ...
        const isRawHTML = htmlPatterns.some(pattern => pattern.test(content));
        
        if (isRawHTML) {
            console.warn('⚠️ Detected raw HTML/CSS content, cleaning...');
            
            // If it's raw HTML, provide a fallback message
            return `I apologize, but I encountered a technical issue processing your request. The system appears to have returned raw HTML code instead of a proper response.

## What You Can Do:

- **Try rephrasing your question** - Sometimes rewording helps
- **Ask a more specific question** - Focus on one topic at a time
- **Check your internet connection** - Ensure you have a stable connection
- **Refresh the page** - This can resolve temporary issues

## Common Topics I Can Help With:

- IEP meeting preparation and guidance
- Understanding special education rights under IDEA
- Finding local resources and support services
- Navigating evaluations and accommodations
- Addressing specific challenges your child faces

Please try asking your question again, and I'll do my best to provide a helpful response.`;
        }
        ...
        cleaned = cleaned.replace(/<\/?(?:html|head|body|style|script|meta|link|title)[^>]*>/gi, '');
